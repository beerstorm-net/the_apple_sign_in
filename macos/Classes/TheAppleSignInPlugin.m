...
// adapted from the original repo https://github.com/tomgilder/flutter_apple_sign_in
@implementation TheAppleSignInPlugin
{
    FlutterEventSink _eventSink;
}

typedef void(^CredentialStateCompletionBlock)(ASAuthorizationAppleIDProviderCredentialState credentialState, NSError * _Nullable error) API_AVAILABLE(macos(10.15));
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    ...
    [registrar registerViewFactory:appleIdButtonFactory
                            withId:@"net.beerstorm/apple_id_button"];

    FlutterMethodChannel* methodChannel = [FlutterMethodChannel methodChannelWithName:@"net.beerstorm/apple_sign_in"
                                                                      binaryMessenger:[registrar messenger]];

    FlutterEventChannel* eventChannel = [FlutterEventChannel eventChannelWithName:@"net.beerstorm/apple_sign_in_events"
                                                                  binaryMessenger:[registrar messenger]];

    TheAppleSignInPlugin* plugin = [[TheAppleSignInPlugin alloc] init];
...
- (void)performRequests:(FlutterMethodCall*)call result:(FlutterResult)result API_AVAILABLE(macos(10.15)) {
...
- (ASAuthorizationAppleIDRequest*)createAppleIDRequest:(NSDictionary*)args API_AVAILABLE(macos(10.15)) {
...
- (void)getCredentialState:(FlutterMethodCall*)call result:(FlutterResult)result API_AVAILABLE(macos(10.15)) {
...
- (void)onCredentialRevokedNotification:(NSNotification *)notification API_AVAILABLE(macos(10.15)) {
    if (_eventSink != nil) {
